...
function DeleteTest({ currentDeleteTestId, refreshTest }) {
  ...
  const createDomain = useMutation(
    (param) =>
      axiosInstance.delete(
        `${import.meta.env.VITE_BACKEND_URL}/api/v1/test?testId=${param}`
      ),
    {
      onSuccess: (response) => {
  ...
  const deleteEntrie = () => {
    // Lancer la mutation pour supprimer le teste
    if (currentDeleteTestId) {
      createDomain.mutate(currentDeleteTestId);
    }
  };
  ...
  useEffect(() => {
    if (
      currentDeleteTestId &&
      currentDeleteTestId !== prevDomainIdRef.current
    ) {
      prevDomainIdRef.current = currentDeleteTestId;
...
DeleteTest.propTypes = {
  currentDeleteTestId: PropTypes.number,
