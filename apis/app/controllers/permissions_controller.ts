...
export default class PermissionsController {
  async allPermission({ auth, bouncer, response }: HttpContext) {
    ...
    if (await bouncer.with('PermissionPolicy').denies('view')) {
      return response.forbidden("Désolé, vous n'êtes pas autorisé lire à les permissions.")
    }

    const query = Permission.query()

    const domains = await query.orderBy('id', 'asc').preload('Profil').paginate(1, 50)
  ...
  async show({ auth, bouncer, request, response }: HttpContext) {
    try {
      ...
      const permission = await Permission.query()
        .preload('Profil') // Précharge la relation 'profil'
  ...
  public async update({ auth, bouncer, request, response }: HttpContext) {
    ...
    try {
      ...
      const userConnected = await User.query()
        ...
        .preload('Profil') // Préchargement du profil associé à l'utilisateur
        .firstOrFail()
